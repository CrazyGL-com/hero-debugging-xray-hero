...
type State = {
	...
	camOffsetTarget: { x: number; y: number };
	scanXSmoothed: number;
	scanXTarget: number;
	autoPhase: number;
	lastPointerActiveAt: number;
	lastInputX: number;
	lastInputY: number;
	lastSize: { w: number; h: number };
};

function DebuggingXrayInner(props: HeroComponentProps) {
	const {
		size,
		input,
		reducedMotion,
	...
	React.useEffect(() => {
		...
		const urls: string[] = [];
		if (screenshot) urls.push(screenshot);
	...
	React.useEffect(() => {
		...
		(async () => {
			...
			const bgMat = new THREE.ShaderMaterial({
				...
				vertexShader:
					...
					'}',
				fragmentShader:
					'/* Coordinate spaces: vUv [0..1]² for a fullscreen quad. */\n' +
					'precision highp float;\n' +
					'varying vec2 vUv;\n' +
					'uniform vec3 u_top;\n' +
		...
		return () => {
			...
			const state = stateRef.current;
			try {
				if (state.planeMesh) state.scene?.remove(state.planeMesh);
	...
	React.useEffect(() => {
		...
		if (!state.ready || !state.planeMat || !screenshot) {
			if (state.ready && !screenshot) setAssetsReady(true);
			return;
		}
	...
	React.useEffect(() => {
		const state = stateRef.current;
		if (!state.ready || !state.planeMat) return;
		const THREE = state.THREE;
		...
		loader.load(
			...
			(tex: any) => {
				...
				tex.wrapT = THREE.ClampToEdgeWrapping;
				tex.anisotropy = state.renderer.capabilities.getMaxAnisotropy?.() ?? 1;
				if (state.xrayTex) state.xrayTex.dispose?.();
				state.xrayTex = tex;
	...
	React.useEffect(() => {
		const state = stateRef.current;
		if (!state.ready || !state.renderer || !state.camera) return;
	...
	return (
		<>
			...
			>
				<canvas ref={canvasRef} className="crazygl-debugging-xray-canvas" aria-hidden="true" />
			</crazygl-stage>
			<crazygl-content>
				<div
					className="crazygl-debugging-xray-content"
					data-content-side={screenshotX >= 0 ? 'left' : 'right'}
				>
					{content.node}
					{(props as any).contentType !== 'custom' && ctaLabel ? (
						<div className="crazygl-debugging-xray-cta-row">
							<button
								ref={ctaRef}
								type="button"
								onClick={handleCTAClick}
								className="crazygl-debugging-xray-cta"
								style={
									{
...
function overlayStyleToFloat(v: string) {
	if (v === 'bboxes') return 0;
	if (v === 'heatmap') return 1;
	return 2; // both
}

export { metadata };
export default function DebuggingXrayHero(props: any) {
	return <CrazyGLWrapper hero={DebuggingXrayInner} metadata={metadata as any} {...props} />;
}
